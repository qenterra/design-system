# Documentation

## Use Design System

- [Master reference](MASTER.md) and [Russian reference](MASTER_RU.md): normative design, behavior, accessibility, and governance.
- [Component catalog](COMPONENT_CATALOG.md) and [Russian catalog](COMPONENT_CATALOG_RU.md): component contracts and delivery status.
- [Consumer adoption](CONSUMER_ADOPTION.md) and [Russian adoption](CONSUMER_ADOPTION_RU.md): packages, manifests, exceptions, and evidence.

## Maintain and release

- [Architecture](ARCHITECTURE.md)
- [Building and verification](BUILDING.md)
- [Dependencies](DEPENDENCIES.md)
- [Testing](TESTING.md)
- [Maintenance](MAINTENANCE.md)
- [Releasing](RELEASING.md)
- [Package release](PACKAGE_RELEASE.md)
- [Deprecation](DEPRECATION.md)
- [Evolution](governance/EVOLUTION.md)
- [Public delivery](governance/PUBLIC_DELIVERY.md)
- [Troubleshooting](TROUBLESHOOTING.md)
- [Requirements](REQUIREMENTS.md)

## Specialized contracts

- [Code system](CODE.md) and [Russian code system](CODE_RU.md)
- [Product releases](RELEASES.md) and [Russian product releases](RELEASES_RU.md)
- [Repository structure](REPOSITORY_STRUCTURE.md), [ownership](OWNERSHIP.md), [style](STYLE_GUIDE.md), and [GitHub settings](GITHUB_SETTINGS.md)
- [Nyx and brand governance](brand/MASTER.md)
- [Figma handoff](FIGMA.md) and [Russian handoff](FIGMA_RU.md)
- [Decision records](decisions/)

Exact values live in `tokens/*.json`; machine-readable truth lives in `registry/` and `schemas/`. Generated adapters and handoff files are outputs, never competing authority.
