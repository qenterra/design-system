...
- Provenance: `registry/packages.json` is the export allowlist; `packages/release-manifest.json` records every public file, byte size, and SHA-256 digest.
...
4. Run `python3 scripts/verify_release.py` inside the exported tree and inspect `npm pack --dry-run --json` plus Swift tests.
