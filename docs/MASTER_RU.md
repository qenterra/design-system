...
Версия 1.0.2 · Нормативный справочник для людей
...
Нативная таблица macOS сохраняет одну иерархию AppKit. `NativeMediaTableCell.configure` сообщает фактически выполненные content/layout изменения; `configurePlaceholder` не создаёт фиктивную identity. Публикация готовой обложки принимает проверенный нормализованный `contentsRect` и отвергает устаревшее поколение запроса. Menu callbacks получают исходный `NSEvent` и настоящую кнопку `NSButton` для привязки меню. Selection, загрузка, кеш и построение меню остаются у потребителя. Presentation явно выбирает другую конечную ширину favorite control, primary tint и потребительскую accessibility-терминологию; defaults не меняются. `mediaExplicitBadge` — отдельная роль компактного однобуквенного значка, не вспомогательного текста.

`ArtworkAccentGradientPrewarmer.prepare()` заранее компилирует библиотеку Metal вне UI-потока; renderer использует ту же библиотеку. Необязательный prewarm не меняет эталонный рельеф, переход палитры, оттенок эффекта или непрозрачный fallback.

Доступность градиента определяется единственной сохраняемой native-поверхностью до первого кадра SwiftUI; начальный tint не ждёт следующего оборота event loop. Стандартные строки ресурсов About наследуют типографику и рендеринг символов из окружения, а оформление Cadence включается явно. Действия feedback учитывают переданный стиль; recovery по умолчанию остаётся secondary.

`ArtworkAccentGradient` накладывает затемнение и оттенок через нативное смешивание цветов SwiftUI поверх чистого Metal-рельефа. `ArtworkAccentGradientView` сохраняет композицию слоёв AppKit для прямых native consumers. Не дублируйте overlay в приложении. Обе поверхности сохраняют непрозрачный fallback; палитра SwiftUI меняется за 0.8 секунды, opacity эффекта использует переданную симметричную длительность, а Reduced Motion отключает анимации.

### 6.7 Представление обложек

Семейство обложек для macOS обрамляет готовое содержимое от потребителя и явно представляет состояния `content`, `loading`, `placeholder` и `error`. Общий код владеет синтетической заглушкой, семантической границей, окном кадрирования и ограниченным преобразованием, геометрией мозаики из нуля–четырёх элементов и декоративным свечением готовой палитры. Декодирование, доступ к файлам и сети, cache и request identity, жесты, сохранение и отмена остаются у потребителя.

Accessible label сохраняет полное название от потребителя. Increased Contrast усиливает границы без изменения геометрии. Reduced Transparency убирает декоративное свечение и полупрозрачный highlight заглушки. Декоративные слои заглушки и свечения не перехватывают ввод и не создают отдельные accessibility elements.
...
Локальная команда выравнивания версии принимает только простой
`MAJOR.MINOR.PATCH`. Она в подготовленной транзакции обновляет все объявленные
канонические поверхности версии, заново создаёт производные и публичные manifest-файлы
и выводит изменённые пути; команда не делает commit, tag, push, публикацию и не
разрешает релиз. Нативные snapshot-тесты используют синтетические данные и точный
профиль OS/architecture. Запись выполняется отдельным точечным запуском с последующей
проверкой каждого PNG в исходном размере; полный gate работает только в режиме
сравнения и покрывает core, media, native table, Reduced Motion, fallback и
фиксированные временные границы gradient.
...
`Sources/ReUI/` — пятый неизменяемый референсный каталог,
исключённый из npm- и SwiftPM-targets. Его живая граница — каждый
бесплатный `c-*` block, публичный `registry:ui` primitive и
`registry:hook`, объявленные индексами Base UI Nova и Radix UI Nova.
Опубликованные в репозитории payloads зафиксированы на одном полном
официальном Git commit; добавления или изменённые payloads, которых там
ещё нет, зафиксированы на неизменяемом live Vercel deployment, общем
для обоих индексов. Manifest хранит hashes обоих индексов, каждый
origin URL, byte count, SHA-256, dependencies, target path и точную
MIT-лицензию с `Copyright (c) 2025 Keenthemes Inc`. ReUI Pro blocks,
платные icons, templates, сайт, docs application, media и build tooling
исключены. `scripts/reui.py sync --write` обновляет каталог только
из этих официальных источников, а `sync --check` сначала сравнивает
сохранённые deployment/index identity и только потом скачивает
изменившиеся payloads. Адаптированная или токенизированная
...
8. Храните операционную координацию вне продуктового репозитория; не дублируйте значения токенов.
...
## 20. Готовность продукта к внедрению
