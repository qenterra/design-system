...
Версия 3.0.0 · Нормативный справочник для людей и AI-агентов
...
QDS стандартизирует семантику, поведение, доступность и доказательства. Единую оболочку приложения система не задаёт.
...
Интерфейсные иконки берутся из `registry/icons.json`: один semantic ID, категория, meaning, имя SF Symbol и minimum OS. Apple-platform consumers используют системный символ напрямую. Generated HTML previews служат доказательством локального прототипа macOS-приложения и не являются распространяемым source artwork. Символ не заменяет visible label при неоднозначном действии или результате.
