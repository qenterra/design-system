...
Версия 1.11.0 · Нормативный справочник для людей и AI-агентов
...
### 12.7 Письма, которыми управляет человек

Email-шаблоны — производственный инструмент для человека, а не автономные сообщения. `registry/email-templates.json` хранит двуязычные сценарии и контракты переменных; `docs/EMAIL.ru.md` хранит полные правила написания, копирования, безопасности и QA.

- `contact@qenterra.com` используется для общих обращений, партнёрств, прессы и прочей переписки не о продукте. `support@qenterra.com` используется для поддержки продуктов, аккаунтов, операций, безопасности и платежей.
- Английская и русская версии семантически равны. Текст должен звучать естественно, а не буквально, но нельзя добавлять или убирать обещание, предупреждение, действие, срок или путь восстановления.
- Локальный конструктор ничего не отправляет, не сохраняет введённые значения, не делает внешних запросов и не поддерживает маркетинг или рассылки.
- Заполняйте только проверенные факты. Нельзя выдумывать результат, причину, срок, событие аккаунта, состояние платежа или вывод о безопасности.
- Язык письма выбирается отдельно от языка справочника. Его смена сохраняет текущие значения и одновременно обновляет названия шаблонов, поля, превью, тему и копируемый результат.
- При смене шаблона сохраняются только значения полей, используемых обоими сценариями; остальные покидают активное состояние в памяти. Очистка удаляет все текущие значения и превью.
- Каждый результат содержит конкретную тему, скрытый прехедер, один ясный заголовок, каноническое приветствие без повторения результата, короткие абзацы с действиями, необязательные структурированные детали, не более одного основного действия, plain-text-версию, ответственный адрес и объяснение получения письма.
- Ссылки действий должны быть абсолютными HTTPS URL. Запрещены пароли, секреты восстановления, полные данные банковских карт, пиксели аналитики, внешние изображения, скрипты, формы и вложения.
- Копирование форматированного текста не доказывает отправку или корректный рендеринг. Перед отправкой человек проверяет тему, получателей, имена, даты, ссылки, суммы, утверждения и итоговое письмо в почтовом клиенте.
...
### 18.6 Приватная дистрибуция пакетов

Канонический репозиторий дизайн-системы выпускает две приватные и неизменяемые
SemVer-дистрибуции: отфильтрованный репозиторий
`qenterra/design-system-swift` и GitHub-пакет `@qenterra/design-tokens`.
Production-потребители используют версионированные удалённые зависимости;
локальные пути допустимы только для согласованной работы над QDS. Публикация
требует совпадения версий, проверенных allowlist содержимого, полного gate и
разрешения чистыми consumer-проектами. Учётные данные имеют минимальные права и
не входят в пакет. Релиз адаптеров не доказывает нативную отрисовку продукта или
приёмку доступности.
