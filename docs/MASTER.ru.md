...
- Чувствительные и финансовые поля очищаются при смене шаблона. В текущей сессии страницы может сохраняться только публичное название продукта.
- Каждый результат содержит конкретную тему, скрытый прехедер, один ясный заголовок, короткие абзацы, необязательные структурированные детали, не более одного основного действия, plain-text-версию, ответственный адрес и объяснение получения письма.
