...
All packages use one Semantic Versioning value. The current release is `5.1.0`; npm metadata, Swift source compatibility, source catalogs, release tags, changelog, and `release-manifest.json` move together. Tags use `v<version>` and published versions are immutable.
...
Copyright 2026 Nikita Melnychenko (QenTerra). QenTerra-authored material is licensed under [Apache-2.0](LICENSE); preserve [NOTICE](NOTICE) when redistribution requires it. Explore SwiftUI examples retain their original ownership and separate permission terms described in [their README](Sources/ExploreSwiftUI/README.md) and [third-party notices](THIRD_PARTY_NOTICES.md).
