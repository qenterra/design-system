...
| Hash or size mismatch | Inspect the named file and manifest record | Hand edit or stale generated output | Regenerate the package snapshot; never patch only the hash |
...
| Repository-hygiene failure | Read the exact audit path and rule | Cache, build output, private operational file, secret, or local path | Move temporary work outside the repository and remove the residue |
...
Do not weaken a verifier, ignore a forbidden path, rewrite a released tag, or copy unrelated repository source into a package target as a workaround. If the published snapshot is wrong, correct canonical source and release a new version.
